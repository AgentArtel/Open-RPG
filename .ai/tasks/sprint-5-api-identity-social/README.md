# Sprint 5 — API-as-Identity + Social + Memory

**Phase**: 6
**Status**: BACKLOG
**Agent**: cursor (018–020), lovable (021)

Photographer NPC with Gemini image-generation skill (proof of API-as-Identity), NPC content store with semantic tagging, associative recall for environment-driven memory, and the Lovable social feed UI.

| Task | Title | Agent | Status |
|------|-------|-------|--------|
| TASK-018 | Photographer NPC + Image Generation Skill | cursor | PENDING |
| TASK-019 | NPC Content Store + Semantic Tagging + Social Posts | cursor | PENDING |
| TASK-020 | Associative Recall + Environment-Driven Memory | cursor | PENDING |
| TASK-021 | Lovable Feed UI + Instagram Bridge | lovable | PENDING |

**Idea docs**: `.ai/idea/08-api-as-identity-npcs.md`, `.ai/idea/09-npc-social-memory-fragments.md`
**Implementation plans**: `.ai/idea/08a-api-powered-skills-implementation-plan.md`, `.ai/idea/09a-social-memory-fragments-implementation-plan.md`
